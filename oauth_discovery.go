...
// oauthMetadataURLs returns metadata URLs for an issuer, RFC 8414 form first.
// RFC 8414 inserts the well-known segment between the origin and the issuer
// path, so https://host/api is described at
// https://host/.well-known/oauth-authorization-server/api. Deployments commonly
// also serve the appended form, and for a path-less issuer the two are equal.
func oauthMetadataURLs(base string) []string {
	appended := base + wellKnownOAuthPath
	u, err := url.Parse(base)
	if err != nil {
		return []string{appended}
	}
	path := strings.TrimSuffix(u.Path, "/")
	inserted := u.Scheme + "://" + u.Host + wellKnownOAuthPath + path
...
func fetchOAuthMetadata(ctx context.Context, metadataURL, base string) *oauthServerMetadata {
