...
func TestRunGetURLWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestRunQueryV2WithOptionalParams(t *testing.T) {
	...
	_, err := client.Runs.QueryV2(context.TODO(), langsmith.RunQueryV2Params{
		...
		Selects:            langsmith.F([]langsmith.RunSelectField{langsmith.RunSelectFieldID, langsmith.RunSelectFieldName, langsmith.RunSelectFieldProjectID, langsmith.RunSelectFieldStartTime, langsmith.RunSelectFieldRunType, langsmith.RunSelectFieldStatus}),
