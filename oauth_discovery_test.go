...
// RFC 8414 inserts the well-known segment before the issuer path; a deployment
// serving only that form must still be discovered.
func TestResolveTokenEndpoint_RFC8414PathInsertedForm(t *testing.T) {
	var srv *httptest.Server
	srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/.well-known/oauth-authorization-server/api" {
			w.Header().Set("Content-Type", "application/json")
			_ = json.NewEncoder(w).Encode(map[string]any{
				"issuer":                        srv.URL + "/api",
				"device_authorization_endpoint": srv.URL + "/api/oauth/device/code",
