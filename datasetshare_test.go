...
func TestDatasetShareNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Datasets.Share.New(
		...
		langsmith.DatasetShareNewParams{
			ShareProjects: langsmith.F(true),
		},
	)
	if err != nil {
...
func TestDatasetShareGet(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestDatasetShareDeleteAll(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
